...
###########
    ...
    def checkConnectiontUser(self, user_id, seq_number, key):
        if not self.serverProxy.getUserById(user_id) is None :        
            if seq_number == self.seq_number_users[key][0] :
                ...
                reactor.callLater(self.delay_to_disconnect, self.checkConnectiontUser, user_id, self.seq_number_users[key][0],key)
...
###########
    ...
    def datagramReceived(self, datagram, host_port):
        ...
        resend, packet = self.lostResponse(user_id, seq_number,host_port)                
            ...
            if message_type == 0x00 :                                                                      # On a reçu le message de login
                ...
                if self.loginRules(new_username) == 1:                                                     # Il faut passer par les régles du serveur 
                    if  self.serverProxy.userExists(new_username) :                                        # Il y a déjà quelqu'un avec le même username
                        ...
                        self.seq_number_users[host_port] = [0,packet,user_id_login]      # On va créer une position pour le user seq_number
                ...
                if status == 0 :
                    reactor.callLater(self.delay_to_disconnect, self.checkConnectiontUser, user_id_login, self.seq_number_users[host_port][0], host_port)
                        
            ###########        
            
            if host_port in self.seq_number_users :                                                                             # On vérifie si le clé existe dans le dictionary                 
                if self.seq_number_users[host_port][2] == user_id and seq_number == self.seq_number_users[host_port][0] + 1 :   # Authentification par la bonne clé seq_number~user_id
